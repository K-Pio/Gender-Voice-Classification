    ...
    int T_out)
{
    ...
    // centralny indeks wejścia dla tego t_out: t_in0 = t_out*stride - pad
    ...
    int T_out)
{
    ...
    for (int co = 0; co < C_out; ++co) {
        ...
        // rozwiązać t_out dla danego t_in i k: t_in = t_out*stride - pad + k*dil
        // -> t_out = (t_in + pad - k*dil) / stride
        #pragma unroll
        for (int k = 0; k < K; ++k) {
